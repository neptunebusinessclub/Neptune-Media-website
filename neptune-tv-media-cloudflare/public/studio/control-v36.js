...
function redirectLegacyHash(){const hash=decodeURIComponent(location.hash.slice(1));if(ADVANCED_TABS.has(hash))location.replace(`/studio/advanced.html#${encodeURIComponent(hash)}`);}
...
function bind(){
  ...
  $('#login').addEventListener('submit',async(event)=>{
    event.preventDefault();const form=new FormData(event.currentTarget),button=$('#loginSubmit');button.disabled=true;$('#authMsg').textContent='Connexion sécurisée…';
    try{
      if(resetToken){if(form.get('password')!==form.get('confirmPassword'))throw new Error('passwords_do_not_match');await api('/api/auth/reset-password',{method:'POST',body:JSON.stringify({token:resetToken,password:form.get('password')})},false);history.replaceState({},'','/studio/');}
      const result=await api('/api/auth/login',{method:'POST',body:JSON.stringify({email:form.get('email'),password:form.get('password')})},false);csrfToken=result.csrfToken||'';sessionStorage.setItem('neptune_csrf',csrfToken);await load();
    }catch(error){$('#authMsg').textContent=humanError(error.message);$('#authMsg').className='message error';}finally{button.disabled=false;}
  });
  $('#requestReset').addEventListener('click',async()=>{
    ...
    try{const email=String($('#login [name=email]').value||'contact@neptunebusiness.com').trim().toLowerCase();await api('/api/auth/request-reset',{method:'POST',body:JSON.stringify({email})},false);$('#authMsg').textContent='Lien envoyé. Il expire dans 20 minutes.';$('#authMsg').className='message success';}
    catch(error){$('#authMsg').textContent=humanError(error.message);$('#authMsg').className='message error';}finally{button.disabled=false;}
  });
  $('#logout').addEventListener('click',logout);$('#refresh').addEventListener('click',()=>load({manual:true}));$('#toastUndo').addEventListener('click',undoLastAction);
  document.addEventListener('visibilitychange',()=>{if(document.hidden){clearTimeout(refreshTimer);return;}scheduleRefresh(400);});
}

async function load({manual=false}={}){
  clearTimeout(refreshTimer);setSync('Vérification…');
  try{
    const auth=await api('/api/auth/status',{},false);csrfToken=auth.csrfToken||csrfToken;sessionStorage.setItem('neptune_csrf',csrfToken);$('#auth').hidden=true;$('#app').hidden=false;
    const role=auth.user?.role||'analyst';$('#accountName').textContent=auth.user?.fullName||auth.user?.email||'Neptune Media';$('#accountRole').textContent=ROLE_LABELS[role]||role;$('#roleBadge').textContent=ROLE_LABELS[role]||role;
    await api('/api/admin/autopilot/reconcile',{method:'POST',body:'{}'}).catch(()=>null);snapshot=await api('/api/admin/control-room');render(snapshot);setSync(manual?'Vérifié maintenant':'Synchronisé');scheduleRefresh(60000);
  }catch(error){
    if(['unauthorized','http_401'].includes(error.message)){ $('#auth').hidden=false;$('#app').hidden=true;$('#authMsg').textContent='';return; }
    setSync('Contrôle incomplet',true);$('#content').innerHTML=`<div class="loading-card"><strong>Impossible de vérifier le Studio</strong><p>${esc(humanError(error.message))}</p><button class="secondary" type="button" data-retry>Réessayer</button></div>`;$('[data-retry]')?.addEventListener('click',()=>load({manual:true}));
  }
}

function render(data){
  const actions=data.actions||[],health=data.health||[],systemIssues=health.filter((item)=>item.level!=='ok').length,totalAttention=actions.length+systemIssues,critical=actions.filter((item)=>item.severity==='critical').length+health.filter((item)=>item.level==='error').length,stateClass=critical?'error':totalAttention?'attention':'ok';
  ...
  const stateText=totalAttention?'Le Studio a isolé uniquement les exceptions qui nécessitent une décision humaine. Tout le reste continue automatiquement.':'Les parcours, les délais, les e-mails et les intégrations sont surveillés. Vous pouvez fermer le Studio.';
...
async function runAction(actionId,button){const item=(snapshot?.actions||[]).find((entry)=>entry.id===actionId);if(!item)return;button.disabled=true;button.textContent='Validation…';try{if(item.type==='supplier'){await api('/api/admin/supplier-payment',{method:'POST',body:JSON.stringify({paymentId:item.paymentId})});showToast('Paiement fournisseur marqué comme effectué.');}else{if(item.externalUrl)window.open(item.externalUrl,'_blank','noopener');const result=await api('/api/admin/autopilot/action',{method:'POST',body:JSON.stringify({orderId:item.orderId,action:item.actionKey})});showToast(result.warning?'Étape validée, mais l’e-mail doit être contrôlé.':'Décision validée. Le parcours continue automatiquement.',false,result.historyId||'');}await load({manual:true});}catch(error){showToast(humanError(error.message),true);button.disabled=false;button.textContent='Réessayer';}}
async function undoLastAction(){if(!pendingUndoId)return;const button=$('#toastUndo');button.disabled=true;try{const result=await api('/api/admin/autopilot/undo',{method:'POST',body:JSON.stringify({historyId:pendingUndoId})});pendingUndoId='';showToast(result.warning?'Action annulée, e-mail à contrôler.':'Action annulée. Le dossier est revenu à l’état précédent.');await load({manual:true});}catch(error){showToast(humanError(error.message),true);}finally{button.disabled=false;}}
...
async function api(url,options={},addCsrf=true){const headers={Accept:'application/json',...(options.headers||{})};if(options.body)headers['Content-Type']='application/json';if(addCsrf)headers['X-CSRF-Token']=csrfToken;const response=await fetch(url,{...options,headers,credentials:'same-origin'});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||`http_${response.status}`);return data;}
function deadlineLabel(value){const date=new Date(value);if(Number.isNaN(date.getTime()))return '';const ms=date.getTime()-Date.now(),abs=Math.abs(ms),days=Math.floor(abs/86400000),hours=Math.max(1,Math.ceil((abs%86400000)/3600000));return ms<0?`Retard ${days?`${days} j`:`${hours} h`}`:days?`Dans ${days} j`:`Dans ${hours} h`;}
function formatDate(value){const date=new Date(value||'');return Number.isNaN(date.getTime())?'Date inconnue':new Intl.DateTimeFormat('fr-FR',{weekday:'short',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}).format(date);}

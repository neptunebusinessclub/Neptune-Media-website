...
export function bookingLink(episode) {
  const url = new URL(BOOKING_URL);
  url.searchParams.set('utm_source', 'webtv');
  url.searchParams.set('utm_medium', 'episode_page');
  url.searchParams.set('utm_campaign', 'neptune_media');
  url.searchParams.set('episode', episode.slug || episode.id);
  return url.toString();
}
...
function header() { return `<header class="site-header"><div class="container header-inner"><a class="brand" href="/" aria-label="Neptune Media, accueil"><img src="/assets/logo-neptune.svg" alt="Neptune Business & Média"></a><nav class="seo-nav"><a href="/emissions/">Émissions</a><a href="/programmes/hors-norme/">Hors Norme</a><a href="/programmes/concept-libre/">Concept Libre</a><a class="btn btn-primary" href="${BOOKING_URL}">Voir les créneaux</a></nav></div></header>`; }
function footer() { return `<footer class="footer"><div class="container"><div class="footer-grid"><div class="footer-brand"><img src="/assets/logo-neptune.svg" alt="Neptune Business & Média"><p>La Web TV des histoires d’entreprise qui méritent l’antenne.</p></div><div class="footer-col"><h3>Explorer</h3><a href="/emissions/">Toutes les émissions</a><a href="/programmes/hors-norme/">Hors Norme</a><a href="/programmes/concept-libre/">Concept Libre</a></div><div class="footer-col"><h3>Informations</h3><a href="/mentions-legales/">Mentions légales</a><a href="/confidentialite/">Confidentialité</a><a href="/contact/">Contact</a></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Neptune Media.</span><span>Des histoires d’entreprise qui méritent l’antenne.</span></div></div></footer>`; }
function formatDuration(seconds) { const value = Math.max(0, Math.round(Number(seconds || 0))); return value < 60 ? `${value} s` : `${Math.floor(value / 60)} min`; }

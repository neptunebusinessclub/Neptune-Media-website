...
export async function enhanceHtml(response, request, env, mode) {
  ...
  const supplementalStyles = mode === 'public'
    ? '<link rel="stylesheet" href="/upgrade-media.css?v=6"><link rel="stylesheet" href="/landing-conversion.css?v=3"><link rel="stylesheet" href="/styles/visual-polish-v11.css?v=1">'
  ...
  if (!body.includes(script)) body = body.replace(marker, `${extraScripts}<script src="${script}"></script>${marker}${mode === 'public' ? '<script src="/visual-polish-v11.js?v=1"></script>' : ''}`);

...
import {
  ...
  safeFilename,
} from './portal-http-utils.js';
import {
  sendCode,
  sendOrderConfirmation,
  sendAppointmentConfirmation,
} from './portal-email.js';
...
export async function handlePortalPublicRoute(request, env, studio) {
  ...
  if (url.pathname === '/api/client/verify-code' && request.method === 'POST') {
    ...
    if (!response.ok) return json(result, response.status);
    return json(
      { ok: true, client: result.client },
      200,
      { 'Set-Cookie': clientCookie(result.token, request.url, result.expiresIn) },
    );
  }

  if (url.pathname === '/api/client/session' && request.method === 'GET') {
    return callStore(studio, '/portal/session', { token: clientToken(request) });
  }
  ...
  if (url.pathname.startsWith('/api/client/files/') && request.method === 'GET') {
    ...
    if (file.externalUrl) {
      return new Response(null, { status: 302, headers: { Location: file.externalUrl, 'Cache-Control': 'private, no-store' } });
    }
    return json({ error: 'file_not_found' }, 404);
  }

  if (
    ['/api/webhooks/client-order', '/api/webhooks/conversion'].includes(url.pathname)
    && request.method === 'POST'
  ) {
    ...
    if (url.pathname === '/api/webhooks/conversion') {
      await callStore(studio, '/webhook/conversion', {
        ...
        sessionId: String(source.sessionId || source.client_reference_id || ''),
        episodeId: String(source.episodeId || ''),
        format: normalized.format,
        externalId: normalized.externalPaymentId,
        amountCents: normalized.amountTotal,
    ...
    if (!normalized.email || !normalized.externalPaymentId) {
      return json({ ok: true, conversionRecorded: url.pathname === '/api/webhooks/conversion', portalSkipped: true });
    }
...
function authorizedWebhook(request, env) {
  ...
  return Boolean(env.CONVERSION_WEBHOOK_SECRET)
    && timingSafeEqual(supplied, env.CONVERSION_WEBHOOK_SECRET);
}

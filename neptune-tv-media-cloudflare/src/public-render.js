...
const YOUTUBE_PLAYLIST_ID = 'PLJuK40_tCulU';
const YOUTUBE_PLAYLIST_EMBED = `https://www.youtube.com/embed/videoseries?list=${YOUTUBE_PLAYLIST_ID}&rel=0&modestbranding=1`;

function youtubePlaylistBlock(title = 'Playlist officielle Neptune Media') {
  return `<section class="youtube-playlist" data-youtube-playlist><div class="catalog-section-head"><span class="eyebrow">Source officielle</span><h2>${escapeHtml(title)}</h2><p>Les dernières émissions publiées sur la chaîne YouTube Neptune sont accessibles directement ici.</p></div><div class="youtube-playlist-frame"><iframe src="${YOUTUBE_PLAYLIST_EMBED}" title="${escapeHtml(title)}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div></section><style>.youtube-playlist{margin:clamp(32px,6vw,72px) 0}.youtube-playlist-frame{position:relative;overflow:hidden;width:100%;aspect-ratio:16/9;border-radius:24px;background:#05070d;box-shadow:0 24px 80px rgba(5,8,18,.22)}.youtube-playlist-frame iframe{position:absolute;inset:0;width:100%;height:100%;border:0}@media(max-width:700px){.youtube-playlist-frame{border-radius:16px}}</style>`;
}
...
export function renderDirect(origin, catalog) {
  ...
  const description = 'Regardez Neptune Media en direct et retrouvez la playlist officielle des émissions.';
  const body = `<main id="main-content" class="seo-main live-page live-page-immediate" tabindex="-1"><h1 class="sr-only">Neptune Media en direct</h1>${youtubePlaylistBlock('Le direct et les émissions Neptune Media')}<section class="related"><span class="eyebrow">À la demande</span><h2>Choisir une émission</h2><div class="seo-grid">${cards(origin, catalog, live)}</div></section></main>`;
  return layout({ origin, title: 'Neptune Media en direct — Web TV business', description, canonical: `${origin}/direct/`, structuredData: [{ '@context': 'https://schema.org', '@type': 'BroadcastService', name: 'Neptune Media — Direct', description, url: `${origin}/direct/` }], body });
}
